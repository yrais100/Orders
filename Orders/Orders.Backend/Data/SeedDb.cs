...
namespace Orders.Backend.Data
{
    public class SeedDb
    {
        ...
        public SeedDb(DataContext dataContext, IUsersUnitOfWork usersUnitOfWork, IFileStorage fileStorage)
        {
        ...
        private async Task<User> CheckUserAsync(string document, string firstName, string lastName, string email, string phone, string address, string image, UserType userType)
        {
            ...
            if (user == null)
            {
                ...
                if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                {
                    filePath = $"{Environment.CurrentDirectory}\\Images\\user\\{image}";
                }
                else
                {
                    filePath = $"{Environment.CurrentDirectory}/Images/user/{image}";
                ...
                var fileBytes = File.ReadAllBytes(filePath);
                var imagePath = await _fileStorage.SaveFileAsync(fileBytes, "jpg", "user");
